...
export function Products() {
  ...
  return <div className="page page-wide">
    ...
    <section className="panel table-panel"><div className="table-caption"><div><strong>SKU 列表</strong><span>共 {rows.length} 条</span></div><SlidersHorizontal size={17} /></div>
      <div className="table-scroll"><table><thead><tr><th>产品 / 链接</th><th>甲方 / 店铺</th><th>ID 状态</th><th>SKU</th><th>售价</th><th>总成本</th><th>毛利</th><th>毛利率</th><th>保本投产</th><th>活动状态</th><th>活动价</th><th>活动毛利率</th><th>活动保本投产</th><th>备注</th><th>操作</th></tr></thead>
        <tbody>{rows.map(sku => { const ctx = getSkuContext(data, sku.id); const activityActive = sku.activity !== '无活动' && sku.activityPrice > 0; const cost = totalCost(sku); return <tr key={sku.id}>
          ...
          <td className="emphasis">{money(sku.salePrice)}</td><td>{money(totalCost(sku))}</td><td className={grossProfit(sku) < 0 ? 'negative' : 'positive'}>{money(grossProfit(sku))}</td><td>{percent(grossMargin(sku))}</td>
  ...
  return <Drawer title={isNew ? '新增 SKU' : sku.name} onClose={onClose}>
    <div className="drawer-content">
      ...
      <section className="form-section"><h3>价格与成本</h3><div className="form-grid">
        {([['salePrice','售价'],['productCost','商品成本'],['shippingCost','快递'],['packagingCost','包装'],['otherCost','其他成本']] as const).map(([key,label]) => <Field key={key} label={label}><Input type="number" min="0" step="0.01" value={sku[key]} onChange={e => number(key, e.target.value)} /></Field>)}
