...
function SkuDrawer({ sku: initial, onClose, onSave, onAdjust }: { sku: Sku; onClose: () => void; onSave: (sku: Sku) => void; onAdjust: (id: string) => void }) {
  ...
  return <Drawer title={isNew ? '新增 SKU' : sku.name} onClose={onClose}>
    <div className="drawer-content">
      ...
      <section className="form-section"><h3>保本投产与活动</h3><div className="form-grid"><Field label="保本投产"><Input value={sku.breakEvenRoi.toFixed(2)} readOnly /></Field><Field label="活动状态"><Select value={sku.activity} onChange={e => { const activity = e.target.value as ActivityType; setSku(current => ({ ...current, activity, activityPrice: activity === '无活动' ? 0 : current.activityPrice })) }}>{activities.map(x => <option key={x}>{x}</option>)}</Select></Field>{sku.activity !== '无活动' && <><Field label="活动价"><Input type="number" min="0" step="0.01" value={sku.activityPrice || 0} onChange={e => number('activityPrice', e.target.value)} /></Field><div className="profit-strip field-full"><div><span>活动毛利</span><strong>{money(sku.activityPrice - totalCost(sku))}</strong></div><div><span>活动毛利率</span><strong>{percent(sku.activityPrice > 0 ? (sku.activityPrice - totalCost(sku)) / sku.activityPrice * 100 : 0)}</strong></div><div><span>活动保本投产</span><strong>{sku.activityPrice > totalCost(sku) ? (sku.activityPrice / (sku.activityPrice - totalCost(sku))).toFixed(2) : '无法持平'}</strong></div></div></>}<Field label="备注" full><Textarea value={sku.remark} onChange={e => set('remark', e.target.value)} /></Field></div></section>
