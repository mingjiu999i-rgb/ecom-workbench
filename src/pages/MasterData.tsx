import { ChevronDown, ChevronRight, ExternalLink, Filter, ListPlus, Pencil, Plus, Search, Trash2 } from 'lucide-react'
import { Fragment, useState } from 'react'
import { Field, Input, Select, Textarea } from '../components/Fields'
import { Modal } from '../components/Overlay'
import { createId, useWorkbench } from '../store/workbench'
import type { Client, Platform, Product, ProductLink, Sku, Store } from '../types/models'
import { calculatedBreakEvenRoi, money, percent } from '../utils/calculations'

type Tab = 'clients' | 'stores' | 'products' | 'links' | 'skus'
type Draft = Partial<Client & Store & Product & ProductLink & Sku> & { platform?: Platform }
...
export function MasterData() {
  ...
  const save = () => {
    ...
    update(current => {
      ...
      if (tab === 'links') { const item: ProductLink = { id: draft.id || createId('link'), clientId: draft.clientId || '', storeId: draft.storeId || '', productId: draft.productId || '', linkId: draft.linkId?.trim() || '', url: draft.url || '', remark: draft.remark || '' }; return { ...current, productLinks: isEdit ? current.productLinks.map(x => x.id === item.id ? item : x) : [...current.productLinks, item] } }
      ...
      return { ...current, skus: isEdit ? current.skus.map(x => x.id === item.id ? item : x) : [...current.skus, item] }
    })
    setDraft(null)
  }
  ...
  const remove = (id: string, label: string) => {
    ...
    update(current => {
      ...
      if (tab === 'links') { const skuIds = new Set(current.skus.filter(x => x.productLinkId === id).map(x => x.id)); return { ...current, productLinks: current.productLinks.filter(x => x.id !== id), skus: current.skus.filter(x => !skuIds.has(x.id)), operations: current.operations.filter(x => !skuIds.has(x.skuId)) } }
      return { ...current, skus: current.skus.filter(x => x.id !== id), operations: current.operations.filter(x => x.skuId !== id) }
    })
  }
