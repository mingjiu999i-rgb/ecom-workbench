...
export function MasterData() {
  ...
  const openNew = () => setDraft(tab === 'stores' ? { clientId: data.clients[0]?.id, platform: '拼多多' } : tab === 'products' ? { clientId: data.clients[0]?.id } : tab === 'links' ? { clientId: data.clients[0]?.id, storeId: '', productId: '' } : {})
  ...
  const valid = tab === 'clients' ? draft?.name?.trim() : tab === 'products' ? draft?.clientId && draft.name?.trim() : tab === 'stores' ? draft?.clientId && draft.name?.trim() : draft?.clientId && draft?.storeId && draft?.productId && draft?.linkId?.trim()
