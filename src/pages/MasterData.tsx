...
export function MasterData() {
  ...
  return <div className="page">
    ...
    <section className="panel master-panel"><div className="tabs" role="tablist">{tabs.map(x => <button role="tab" aria-selected={tab === x.id} className={tab === x.id ? 'active' : ''} key={x.id} onClick={() => setTab(x.id)}>{x.label}<span>{data[x.id === 'links' ? 'productLinks' : x.id].length}</span></button>)}</div><div className="master-content">
      ...
      {tab === 'stores' && <SimpleTable headers={['店铺名称','所属甲方','平台','使用状态','操作']} rows={data.stores.map(x => [<strong>{x.name}</strong>, data.clients.find(c => c.id === x.clientId)?.name || '—', <span className="tag">{x.platform}</span>, <span className={`usage-status usage-${x.storeStatus}`}>{x.storeStatus}</span>, <Actions key="a" onEdit={() => setDraft(x)} onDelete={() => remove(x.id, x.name)} />])} />}
