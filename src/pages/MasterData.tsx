...
import type { Client, LinkStatus, Platform, Product, ProductLink, Sku, Store, StoreStatus } from '../types/models'
...
type Draft = Partial<Client & Store & Product & ProductLink & Sku> & { platform?: Platform; status?: LinkStatus; storeStatus?: StoreStatus }
...
const storeStatuses: StoreStatus[] = ['启用', '暂停']
...
export function MasterData() {
  ...
  const [pausedStoresExpanded, setPausedStoresExpanded] = useState(false)
  ...
  const openNew = () => setDraft(tab === 'stores' ? { clientId: data.clients[0]?.id, platform: '拼多多', storeStatus: '启用' } : tab === 'products' ? { clientId: data.clients[0]?.id, operationRate: 0.02 } : tab === 'links' ? { clientId: data.clients[0]?.id, storeId: '', productId: '', status: '日销' } : tab === 'skus' ? { productLinkId: '', productCostId: '', name: '', salePrice: 0 } : {})
  ...
  const save = () => {
    ...
    update(current => {
      ...
      if (tab === 'stores') { const item: Store = { id: draft.id || createId('store'), clientId: draft.clientId || '', name: draft.name?.trim() || '', platform: draft.platform || '其他', storeStatus: draft.storeStatus || '启用' }; return { ...current, stores: isEdit ? current.stores.map(x => x.id === item.id ? item : x) : [...current.stores, item] } }
      if (tab === 'products') { const item: Product = { id: draft.id || createId('product'), clientId: draft.clientId || '', name: draft.name?.trim() || '', operationRate: Number(draft.operationRate || 0), remark: draft.remark || '' }; return { ...current, products: isEdit ? current.products.map(x => x.id === item.id ? item : x) : [...current.products, item] } }
  ...
  const pausedStores = data.stores.filter(store => store.storeStatus === '暂停')
  const storeRows = (stores: Store[]) => stores.map(store => [<strong>{store.name}</strong>, data.clients.find(client => client.id === store.clientId)?.name || '—', <span className="tag">{store.platform}</span>, <span className={`usage-status usage-${store.storeStatus}`}>{store.storeStatus}</span>, <Actions key="a" onEdit={() => setDraft(store)} onDelete={() => remove(store.id, store.name)} />])
